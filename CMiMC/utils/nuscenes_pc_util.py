...
def from_file_multisweep_upperbound_sample_data(
    ...
    num_sensor = len(sample_record["data"])
    for k in range(num_sensor):
        if no_cross_road and k == 0:    # 跳过第0个agent，是RSU(v2数据集)，处理v1的时候也会存这项，但没关系，train的时候导入数据集设置rsu=1，会输出完整的包括agent 0的数据集
            continue

        # Load up the pointcloud.
        pointsensor_token = sample_record["data"]["LIDAR_TOP_id_" + str(k)]
        current_sd_rec = nusc.get("sample_data", pointsensor_token)
        current_pc = LidarPointCloud.from_file( # (4, 52947)
        ...
        trans_matrix_list.append(trans_matrix)  # trans_matrix (4,4)
        ...
        all_times = np.hstack((all_times, times))   # (1,43275)->(1,43275+51442点数)->...
        all_pc.points = np.hstack((all_pc.points, current_pc.points))   # (4,43275)->(4,43275+51442)->...

    trans_matrix_list = np.stack(trans_matrix_list, axis=0) # (N,4,4) 当前帧有N个agent的N个采样
...
def from_file_multisweep_warp2com_sample_data(
    ...
    # 当前帧里，每个agent的采样，以current_agent的x/y_global为中心的x/y_local
    for k in range(num_sensor):
    ...
    for k in range(num_sensor):
        ...
        # Collect the transformation matrix
        trans_matrix_list.append(trans_matrix)  #(N,4,4)
    ...
    max_num_agent = 5
    for k in range(max_num_agent - num_sensor): # 没有雷达传感器数据的agent，也加进点云转换矩阵list
        trans_matrix_list.append(np.zeros((4, 4)))
        trans_matrix_list_no_cross_road.append(np.zeros((4, 4)))

    trans_matrix_list = np.stack(trans_matrix_list, axis=0) # len=5
...
def get_instance_boxes_multisweep_sample_data(nusc: 'NuScenes',
    ...
    for _ in range(nsweeps_back):   #1
        ...
        if current_sd_rec['prev'] == '':    # 没有前面的ann，current_sd_rec不变
            break
        else:   # 有前面的ann，下面的current_sd_rec取前一个，继续for循环，但是nsweeps_back=1，没什么用了
    ...
    if current_sd_rec['next'] != '':    # current_sd_rec有next，current_sd_rec取next
        current_sd_rec = nusc.get('sample_data', current_sd_rec['next'])

        for _ in range(nsweeps_forward):    # nsweeps_forward=0,不执行
            box, attr, cat = get_instance_box(nusc, current_sd_rec['token'], instance_token)
            boxes.append(box)  # It is possible the returned box is None
            attr_list.append(attr)
            cat_list.append(cat)
